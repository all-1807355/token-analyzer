...
def effective_slippage_rate(address, chain, web3):
    """
    Calculates the effective slippage rate for a token by comparing price impact
    between different trade sizes using Uniswap-style AMM math.
    The objective is to simulate selling the token @ address and check whether the slippage is unusually high
    Let's say:
    You try to sell 100 Frank and expect ~0.5 WBNB.
    You actually receive 0.01 WBNB.
    This is 98% slippage, a red flag.
    """
    address = address.lower()

    pair_info = get_lp_pair(address, chain, web3)
    if pair_info == (None, None):
        debug_print("❌ Could not find liquidity pair")
        return None

    pair_address, pair_abi = pair_info
    token_decimals = get_token_decimals(address, web3)

    price_info = get_price_and_liquidity(
        pair_address,
        address,
        chain,
        web3,
    ...
    if not price_info:
        debug_print("❌ Could not fetch price and liquidity info")
        return None

    current_price = price_info['price_usd']
    liquidity = price_info['liquidity_usd']
    quote_token_price = price_info['quote_token_usd_price']

    if not current_price or not liquidity or not quote_token_price:
        debug_print("❌ Missing price or liquidity info")
        return None

    if current_price == 0:
        debug_print("❌ Current price is zero — cannot compute slippage")
        return None

    # Test trade sizes
    small_trade_usd = liquidity * 0.001
    large_trade_usd = liquidity * 0.01

    small_trade_tokens = small_trade_usd / current_price
    large_trade_tokens = large_trade_usd / current_price

    small_trade_in = int(small_trade_tokens * (10 ** token_decimals))
    large_trade_in = int(large_trade_tokens * (10 ** token_decimals))
    ...
    #The token we want to sell is the token being analyzed -> the token we put IN the pool
    if token0.lower() == address:
        reserve_in = reserves[0]
        reserve_out = reserves[1]
    elif token1.lower() == address:
        reserve_in = reserves[1]
        reserve_out = reserves[0]
    else:
        debug_print("❌ Token not found in LP pair")
        return None

    if reserve_in == 0 or reserve_out == 0:
        debug_print("❌ No liquidity in pool")
        return None

    def get_price_after_trade(dx, reserve_in, reserve_out):
        if dx == 0 or reserve_in + dx == 0:
            return 0
        dy = reserve_out * dx / (reserve_in + dx)
        if dx == 0:
            return 0
        return dy / dx  # quote tokens per token sold

    small_trade_quote = get_price_after_trade(small_trade_in, reserve_in, reserve_out)
    large_trade_quote = get_price_after_trade(large_trade_in, reserve_in, reserve_out)
    small_price_usd = small_trade_quote * quote_token_price
    large_price_usd = large_trade_quote * quote_token_price

    if current_price == 0:
        debug_print("❌ Division by zero risk: current_price is 0")
        return None

    small_slippage = ((current_price - small_price_usd) / current_price) * 100
    large_slippage = ((current_price - large_price_usd) / current_price) * 100
    avg_slippage = (small_slippage + large_slippage) / 2

    # 1. Compute the spot price before trade
    spot_price = reserve_out / reserve_in
    expected_output = amount_in * spot_price

    fee = 0.003 #uniswap base fee of 0.30% we don't take into consideration any contract fees
    # 2. Compute actual output using AMM formula
    amount_in_with_fee = amount_in * (1 - fee)
    actual_output = (amount_in_with_fee * reserve_out) / (reserve_in + amount_in_with_fee)

    # 3. Compute slippage
    slippage_percent = ((expected_output - actual_output) / expected_output) * 100

    # Protect against division by zero in LP metrics
    try:
        low_price_check = (
            current_price < 1e-10 and
            (reserve_out / 1e18) * quote_token_price < 0.1 and
            avg_slippage < 5
        )
    except ZeroDivisionError:
        low_price_check = False

    result = {
        "small_slippage_percent": round(small_slippage, 4),
        "large_slippage_percent": round(large_slippage, 4),
        "avg_slippage_percent": round(avg_slippage, 4),
        "slippage_over_5": avg_slippage > 3.0,
        "slippage_over_10": avg_slippage > 10.0,
        "low_price_for_low_slippage": low_price_check
    }

    if current_price < 1e-10 and reserve_out / (10 ** 18) < 0.1:
        print("⚠️ LP shows typical rug-pull pattern: near-zero price and drained quote token")

    return result

def is_token_suspicious_by_slippage(token_address: str, chain: str, web3, retries: int = 3, retry_delay: float = 1.0):
    import time
    ...
    pair_info = get_lp_pair(token_address, chain, web3)
    if pair_info == (None, None):
        print("❌ Could not find liquidity pair")
        return None
    pair_address, pair_abi = pair_info
    ...
    return {
        ...
        "slippages_by_percent": {
            "slippage_percents": percent_slippages,
            "first_abnormal_slippage": {
                "trade_percent": first_abnormal_percent,
                "slippage_percent": first_abnormal_slippage_percent,
            } if percent_suspicious else None
        },

        "slippages_by_fixed_input": {
            "slippage_percents": fixed_slippages,
            "first_abnormal_slippage": {
                "input_amount": first_abnormal_fixed,
                "slippage_percent": first_abnormal_slippage_fixed,
            } if fixed_suspicious else None
        }
    }
    ...
    for holder in lp_holders:
        for lock in holder.get("locked_detail") or []:
            ...
            try:
                ...
                if (end_time - now).days >= 15: #CHANGE WITH TOKEN START DATE vai a capire se tra le feature ce n'è una dominante per capire se un token è malevolo o no (estrazione feature dominante) da inserire dopo la parte di analisis
